...
class AudioMetricsCalculator:
    ...
    def calculate_activity_windows(self, audio, sr, threshold=-35, min_silence_duration=0.2):
        """
    ...
    def process_file(self, filename):
        ...
        metrics = {
            ...
            'latency_metrics': self.calculate_average_latency(user_windows, agent_windows),
            'agent_answer_latencies': [l * 1000 for l in self.calculate_agent_answer_latency(user_windows, agent_windows)],  # ms
            'ai_interrupting_user': self.detect_ai_interrupting_user(user_windows, agent_windows),
            'user_interrupting_ai': self.detect_user_interrupting_ai(user_windows, agent_windows),
            'talk_ratio': self.calculate_talk_ratio(user_windows, agent_windows),
            'average_pitch': self.calculate_average_pitch(audio, sr),
            'words_per_minute': self.calculate_words_per_minute(audio, sr, agent_windows),
