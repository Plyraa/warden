...
def simulate_elevenlabs_response():
    ...
    class MockWord:
        def __init__(self, text, start, end, word_type="word", logprob=1.1, speaker_id="speaker_1"):
            self.text = text
            self.start = start
            self.end = end
            self.type = word_type
...
def test_merge_and_format_transcript(words_customer=None, words_agent=None):
    ...
    if words_agent is None:
        words_agent = [
            {"text": "Hi", "start": 0.8, "end": 1.0, "speaker": "ai_agent"},  # Overlaps with "there"
...
def test_full_pipeline():
    ...
    if not test_files:
        print("No test files found. Please add files to the stereo_test_calls directory.")
        return
    ...
    if metrics.get("transcript_data"):
        print(f"Transcript generated successfully with {len(metrics['transcript_data']['words'])} words")
        if metrics["transcript_data"].get("has_overlaps"):
            print(f"Detected {metrics['transcript_data'].get('overlap_count', 0)} overlapping words")
        print("Sample dialog:")
        print(metrics["transcript_data"]["dialog"][:200] + "...")  # Show first 200 chars
        print("✓ Full pipeline test passed!")
    else:
        print("No transcript data generated. Check if ElevenLabs API key is configured.")
...
def test_database_storage():
    ...
    try:
        ...
        if not metrics.get("transcript_data"):
            print("× Database test failed: Transcript data not found in retrieved metrics")
            return
        ...
        assert "has_overlaps" in transcript_data, "Transcript should contain has_overlaps"
        assert "overlap_count" in transcript_data, "Transcript should contain overlap_count"
        
        print("✓ Database storage test passed!")
        print(f"Successfully retrieved transcript with {len(transcript_data['words'])} words " +
