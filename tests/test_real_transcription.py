...
def test_real_transcription(customer_file, agent_file):
    ...
    if not calculator.elevenlabs_client:
        print(
            "ERROR: ElevenLabs API key not found. Please set ELEVENLABS_API_KEY in your .env file."
        )
        return
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        customer_path = (
            os.path.abspath(customer_file) if os.path.exists(customer_file) else None
        )
        ...
        transcript = calculator._merge_and_format_transcript(
            customer_words, agent_words
        )
        ...
        overlapping_words = [
            w for w in transcript["words"] if w.get("is_overlap", False)
        ]
        if overlapping_words:
            print("\nSample overlapping words:")
            for i, word in enumerate(overlapping_words[:5]):  # Show up to 5 examples
                print(
                    f"  - '{word['text']}' ({word['speaker']}): {word['start']:.2f}s to {word['end']:.2f}s"
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="Test ElevenLabs transcription with real audio files."
    )
    parser.add_argument(
        "--customer", "-c", required=True, help="Path to customer audio file (mono)"
    )
    parser.add_argument(
        "--agent", "-a", required=True, help="Path to agent audio file (mono)"
    )
