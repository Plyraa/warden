...
def mock_transcribe_test():
    ...
    customer_words = [
        {"text": "Hello", "type": "word", "start": 0.0, "end": 0.5, "speaker_id": "customer"},
        {"text": "how", "type": "word", "start": 1.0, "end": 1.2, "speaker_id": "customer"},
        {"text": "are", "type": "word", "start": 1.3, "end": 1.5, "speaker_id": "customer"},
        {"text": "you", "type": "word", "start": 1.6, "end": 1.8, "speaker_id": "customer"},
        {"text": "today", "type": "word", "start": 1.9, "end": 2.3, "speaker_id": "customer"},
        {"text": "I", "type": "word", "start": 2.5, "end": 2.6, "speaker_id": "customer"},
        {"text": "need", "type": "word", "start": 2.7, "end": 3.0, "speaker_id": "customer"},
        {"text": "help", "type": "word", "start": 3.1, "end": 3.5, "speaker_id": "customer"},
    ]
    
    agent_words = [
        {"text": "Hi", "type": "word", "start": 2.0, "end": 2.2, "speaker_id": "agent"},
        {"text": "there", "type": "word", "start": 2.3, "end": 2.5, "speaker_id": "agent"},
        {"text": "I'm", "type": "word", "start": 2.6, "end": 2.8, "speaker_id": "agent"},
        {"text": "doing", "type": "word", "start": 2.9, "end": 3.2, "speaker_id": "agent"},
        {"text": "great", "type": "word", "start": 3.3, "end": 3.6, "speaker_id": "agent"},
    ]
    ...
    assert result["has_overlaps"] == True, "Failed to detect overlaps"
...
def test_database_storage(transcript_data):
    ...
    try:
        ...
        assert "transcript_data" in recreated, "Transcript data missing from recreated metrics"
        assert "has_overlaps" in recreated["transcript_data"], "Overlap info missing from transcript data"
        assert recreated["transcript_data"]["has_overlaps"] == True, "Overlap flag not stored correctly"
        assert recreated["transcript_data"]["overlap_count"] >= 4, "Overlap count not stored correctly"
        
        print("✓ Successfully retrieved and validated overlap data from database")
        print(f"  - Retrieved overlap count: {recreated['transcript_data']['overlap_count']}")
...
def main():
    ...
    recreated = test_database_storage(transcript_data)
    
    print("\n=== All Tests Passed! ===")
    print("The ElevenLabs transcript processing pipeline is working correctly.")
    print("Overlapping speech is properly detected, stored in the database, and can be visualized.")
