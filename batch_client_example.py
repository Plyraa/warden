...
def main():
    ...
    if choice in ["1", "3"]:
        ...
        files = [f for f in os.listdir(base_dir) if f.endswith(".mp3") or f.endswith(".wav")]
        
        if not files:
            print("No audio files found in stereo_test_calls directory")
        else:
            # Use first 2 files as an example
            sample_files = files[:2]
            local_file_paths = [os.path.join(os.path.abspath(base_dir), f) for f in sample_files]
    ...
    if choice in ["2", "3"]:
        ...
        mp3_urls = [
            "https://filebin.net/3cer9nvkqt1rwuwc/243801406824559add7684.37683750.mp3",
            "https://filebin.net/ofmcpdnwmk1qwfag/66057328368247d623d0b87.67876133.mp3",
        ]
        
        print(f"\nAdding {len(mp3_urls)} URL references:")
        for url in mp3_urls:
            print(f" - {url}")
            file_paths.append(url)  # Add URLs directly - they will be processed by the server
    ...
    if results:
        ...
        for i, result in enumerate(results):
            ...
            print("\nInterruptions:")
            print(f"AI Interrupting User: {'Yes' if result['ai_interrupting_user'] else 'No'}")
            print(f"User Interrupting AI: {'Yes' if result['user_interrupting_ai'] else 'No'}")
